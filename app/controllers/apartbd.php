...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apart-create'])){
  ...
  if($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id_room'])){
    ...
    $id_rooms = $room['id'];
    $categorys = $room['id_category'];
    $number_rooms = $room['number_room'];
    $state = $room['state'];
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['room_edit'])){
  ...
  if ($flag == 0) {
      ...
      $getidap = selectIdCategory('apartments', $category);
      $idap = $getidap['id'];
      $roomsupd = [
          'id_category' => $idap,
...
// создание номера
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['room_create'])){
  ...
  if ($flag == 0) {
    $existence = selectOne('rooms', ['number_room' => $number_room]);
    if ($existence['number_room'] === $number_room){
      $err['number_room'] = '<small class="text-danger">Комната с таким номеров уже есть!</small>';
    } else {
    $getidap = selectIdCategory('apartments', $category);
    $idap = $getidap['id'];
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apart-edit'])){
  ...
  // Удаление номера
  if($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['delroom_id'])){
    $id = $_GET['delroom_id'];
    delete('rooms', $id);
    header('location: ' . BASE_URL . 'rooms-admin.php');
  }